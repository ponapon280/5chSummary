# 🆕 新規トピック（前回からの差分）
- easywan（メモリ32GBで動作、低スペック魅力）
- nano-banana（4コマ漫画生成テスト、pro食べ放題詐欺）
- Forge Classic（言及のみ）
- reForge（anima Checkpoint動かず、GPUデフォルトRNG罠）
- Swarm UI（Forge/ComfyUI代替提案）
- D2 Send Eagle（メタデータ保存/フォルダ分け/タグ付け便利、有償）
- SD Prompt Reader（改良版/Codex併用、Mask編集時メタデータ解決）
...
- SAM3関連カスタムノード具体名（ComfyUI-SAM3, Easy-SAM3, TBG-SAM3, PozzettiAndrea版：Eyes/Background Detailer安定、ループ40枚耐性、VRAM12GB厳しい/sam3.pt 3.21GB、comfy-env/comfy-3d-viewersゴミインストール）
- ComfyUI Core機能（Hook/KeyframesでLoRAスケジュール、カスタムノードなし最小構成）
- ComfyUI Save Image Extended（webp D&Dエラー回避、Node ID/filename_keysでSD風保存、日時/シード挿入）
- Civitai WF（分割・解説付き初心者学習向き、巨大WF難易度）
- ComfyUI自前セーバー/ローダー作成柔軟性
- comfyui-prompt-control（LoRAスケジュールで砂嵐）
- ComfyUI更新時依存破壊（SageAttention/FlashAttention/comfy-env破壊）
- ComfyUIバッチ生成（200枚キューでメモリ圧迫）
- ComfyUI Appモード（使い勝手不足）
- ComfyUI最新版出力劣化（人体破綻増加懸念）
- ComfyUI Anima共同開発
- ComfyUIメタデータ保持変換ソフト需要
- A1111 Chrome最小化中生成継続（ForgeNeo比安定）
- A1111 Gradio 3使用（Forge系問題回避）
- A1111 WSL+ComfyUI併用UI乱れ
- Forge系 RNG CPUデフォルト（速度影響なし）
- Forge系 RTX 5070 Ti 30step約16秒
- Forge Neo Chrome最小化生成停止（Gradio 4問題）
- Forge系生成速度低下（SageAttention/fp16効かず）
- Forge系ハイレゾフィックス不調
